...
open class PPAgreementButton: UIView {
    ...
    public var selectedIcon: UIImage = Res.image("base_blue_selected") {
        didSet { refresh() }
    }
    
    public var unSelectedIcon: UIImage = Res.image("base_blue_unselected") {
        didSet { refresh() }
    }
    
    public var textColor: UIColor = UIColor.fy.black {
        didSet { refresh() }
    }
    
    public var linkColor: UIColor = UIColor.fy.blue {
        didSet { refresh() }
    }
    ...
    /// 是否需要换行缩进
    public var needLineFeedIndent: Bool = false {
        didSet {
            if needLineFeedIndent {
                let paragraphStyle = NSMutableParagraphStyle.init()
                paragraphStyle.lineSpacing = 5
                paragraphStyle.headIndent = iconSize
                self.attributes = [
...
extension PPAgreementButton {
    ...
    private func refresh() {
        let prefix = " " + (protocolPrefix ?? Res.text("我已阅读并同意"))
        let attributeText0 = NSMutableAttributedString.yy_attachmentString(withContent: isChecked ? selectedIcon : unSelectedIcon,
                                                                           contentMode: .center,
                                                                           attachmentSize: CGSize(width: iconSize, height: iconSize),
                                                                           alignTo: UIFont.fy.system_14,
                                                                           alignment: .center)
        attributeText0.yy_setTextHighlight(NSMakeRange(0, attributeText0.string.count), color: textColor, backgroundColor: UIColor.fy.clear) { [weak self] (_,_,_,_) in
            self?.outTap()
        }
        let attributeText1 = NSMutableAttributedString(string: prefix, color: textColor, font: UIFont.fy.system_14)
        attributeText1.yy_setTextHighlight(NSMakeRange(0, prefix.count), color: textColor, backgroundColor: UIColor.fy.clear) { [weak self] (_,_,_,_) in
            self?.outTap()
        }
        let attributeText2: NSMutableAttributedString = {
            ...
            if agreements.count > 1 {
                if multipleDesc?.count ?? 0 > 0 {
                    ...
                    agreements.enumerated().forEach { (index, agreement) in
                        ...
                        let text = NSMutableAttributedString(string: name, color: linkColor, font: UIFont.fy.system_14)
                        text.yy_setTextHighlight(NSMakeRange(0, name.count), color: linkColor, backgroundColor: UIColor.fy.clear) { [weak self] (_,_,_,_) in
                            self?.agreementTap(index: index)
                        }
                        attributedString.append(text)
                        if index < agreements.count - 1 {
                            attributedString.append(NSMutableAttributedString(string: "、", color: textColor, font: UIFont.fy.system_14))
                        }
                    }
            ...
            if let suffix = suffix {
                let text = NSMutableAttributedString(string: suffix, color: linkColor, font: UIFont.fy.system_14)
                text.yy_setTextHighlight(NSMakeRange(0, suffix.count), color: linkColor, backgroundColor: UIColor.fy.clear) { [weak self] (_,_,_,_) in
        ...
        if let attributes = attributes {
            var map = [String : Any]()
            attributes.forEach { map[$0.key.rawValue] = $0.value }
            for attributeText in [attributeText0, attributeText1, attributeText2] {
                let map_ = attributeText.yy_attributes?.merging(map, uniquingKeysWith: { $1 })
                attributeText.setYy_attributes(map_ ?? map)
            }
        } else {
