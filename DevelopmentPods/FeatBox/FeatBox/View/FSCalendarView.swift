...
/// 日历控件
public class FSCalendarView<Cell: FSCalendarViewCellable>: BaseView, FSCalendarDataSource, FSCalendarDelegate, Storyboardable {
    ...
    public lazy var calender: FSCalendar = {
        let calendar = FSCalendar(frame: CGRect(x: 0, y: titleLabelHeight, width: self.frame.size.width, height: calendarHeight))
        ...
        calendar.appearance.headerTitleColor = UIColor.fy.black_333333
        ...
        calendar.register(Cell.self, forCellReuseIdentifier: "CalendarCell")
    ...
    public init(frame: CGRect, calendarHeight: CGFloat, titleLabelHeight: CGFloat) {
    ...
    public required init?(coder: NSCoder) {
        // Swift 要求所有 stored property 在 super.init 调用前赋值;
        // Storyboardable 子类不支持 coder 路径,但仍要先填占位值满足编译器。
    ...
    public func calendar(_ calendar: FSCalendar, cellFor date: Date, at position: FSCalendarMonthPosition) -> FSCalendarCell {
        calendar.dequeueReusableCell(withIdentifier: "CalendarCell", for: date, at: position)
    }
...
extension FSCalendarView {
    ...
    private func setupButtonIconDisabled() {
        ...
        if let nextMonth = self.calender.currentPage.fy.adding(.month, value: 1),
           nextMonth >= maxDate {
...
extension FSCalendarView: Booming.LevelStatusBarWindowShowUpable {

    public func show(animated: Bool, animation: (() -> Void)?, completion: ((Bool) -> Void)?) {
        let animationBlock = { [weak self] in
            let height = (self?.calendarHeight ?? 0.0) + (self?.titleLabelHeight ?? 0.0)
