...
class BannerDetailTopCellViewModel: BaseTableViewCellViewModelable {
    var cellType: FeatBox.BaseTableViewCell.Type {
        BannerDetailTopCell.self
    }
    var index: Int?
...
class BannerDetailTopCell: BaseTableViewCell, HasDisposeBag {
    ...
    let bannersAndIndex = BehaviorRelay<(index: Int?, banners: [Banner])>(value: (0, []))
    
    override var viewModel: BaseTableViewCellViewModelable? {
        didSet {
            guard let viewModel = viewModel as? BannerDetailTopCellViewModel,
                  let banners = viewModel.datasource as? [Banner] else {
                return
            }
            self.bannersAndIndex.accept((viewModel.index, banners))
        }
    }
    ...
    lazy var layout: UICollectionViewFlowLayout = {
        ...
        let width = Constants.width - BannerDetailTopCell.spaceing * 4
    ...
    override func setupBindings() {
        ...
        bannersAndIndex.map { $0.banners }
            .bind(to: collectionView.rx.items) { (collectionView, row, element) in
                ...
                let item = collectionView.fy.dequeueReusableCell(BannerDetailTopCellCollectionViewCell.self, for: indexPath)
                item.banner = element
                return item
            }.disposed(by: rx.disposeBag)
        ...
        self.collectionView.rx.didEndDecelerating.subscribe(onNext: { [weak self] _ in
            ...
            (self?.viewModel as? BannerDetailTopCellViewModel)?.currentIndex.accept(page)
        }).disposed(by: rx.disposeBag)
    }
...
fileprivate class BannerDetailTopCellCollectionViewCell: BaseCollectionViewCell {
    ...
    var banner: Banner? {
        didSet {
            ...
            backImageView.fy.setImage(with: banner.imagePath)
        }
    }
    ...
    lazy var seeCardNoButton: UIButton = {
        ...
        button.setTitleColor(UIColor.fy.black_666666, for: .normal)
