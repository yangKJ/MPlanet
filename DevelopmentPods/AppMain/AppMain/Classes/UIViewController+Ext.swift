...
extension BoxWrapper where Base: UIViewController {
    ...
    private static func popOrDismissToRootViewController(controller: UIViewController?, selectedTabIndex: Int, animated: Bool, completion: (() -> Void)?) {
        ...
        if controller.presentingViewController != nil {
            if animated {
                controller.dismiss(animated: animated, completion: { [weak controller] in
                    let next = controller ?? self.currentViewController()
                    self.popOrDismissToRootViewController(controller: next, selectedTabIndex: selectedTabIndex, animated: animated, completion: completion)
                })
            } else {
